...
//         options.addArguments("--user-agent=Mozilla/5.0 (iPhone; CPU iPhone OS 13_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0 Mobile/15E148 Safari/604.1");
		...
		try (FileInputStream file = new FileInputStream(".\\input-data\\BB28 Input data New.xlsx");
			...
			Sheet urlsSheet = urlsWorkbook.getSheet("Nashik");
				...
				try {
					...
					if (currentPin == null || !currentPin.equals(locationSet)) {
						...
						try {
							...
							button.click();
							try {
								WebElement locationInput = wait11.until(ExpectedConditions.elementToBeClickable(By
...
//                            	  WebElement locationInput = wait11.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@placeholder='Search for area or street name']")));
//
//                                  // Click on the location input field
//                                  locationInput.click();
//                                  locationInput.sendKeys(locationSet);
							...
							WebElement suggestion = wait.until(ExpectedConditions
									.elementToBeClickable(By.xpath("//ul[@class='overscroll-contain p-2.5']//li[1]")));

							// Click on the first suggestion (adjust XPath based on actual suggestion
							// structure)
							suggestion.click();
...
//             			}
	...
	private static void saveResultsToExcel(Workbook resultsWorkbook) {
		try {
			...
			String outputFilePath = ".\\Output\\BB_OutputData_Nashik" + timestamp + ".xlsx";
