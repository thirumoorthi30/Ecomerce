		...
		// TODO Auto-generated method stub
		 //System.setProperty("webdriver.chrome.driver", "./Drivers//chromedriver.exe");
	        ...
	        try {
	            ...
	            String filePath = ".\\input-data\\amazon NY&Chri input.xlsx";
	           				...
	           				//	Thread.sleep(1000);909
	           					...
	           					for (int j = 0; j < 200; j++) {
	           						try {         
	           							...
	           							driver.findElement(
	           									By.xpath("/html/body/div[2]/div/div/div/div/div/div/div[2]/div/div[2]/div[1]/div/input"))
	           									.sendKeys(Keys.ENTER);
	           							
	           							Thread.sleep(3000);
	           							
	           							driver.findElement(
	           									By.xpath("/html/body/div[2]/div/div/div/div/div/div/div[2]/div/div[2]/div[1]/div/input")).clear();
	           ...
	           //sp
	                    Thread.sleep(2000);
	                   try {
	                    WebElement sp = driver.findElement(By.xpath("//div[@class='mt-5']//div[@class='flex items-center gap-2']//span[1]"));
...
//	                   
						 String currentUrl = driver.getCurrentUrl();
				            ...
				            // Find the position of '/pvid/' in the URL and extract the pid
				            int pvidIndex = currentUrl.indexOf("/pvid/");
				            if (pvidIndex != -1) {
				                // Extract the part after '/pvid/' and stop at the first '?'
				                pid = currentUrl.substring(pvidIndex + 6);  // Skip past "/pvid/"
				                
				                // Check if there is a '?' in the URL, to stop there
				                int queryIndex = pid.indexOf("?");
				                if (queryIndex != -1) {
				                    pid = pid.substring(0, queryIndex);  // Extract only the part before '?'
				                }
				                
				                // Print the extracted pid for debugging
				                System.out.println("Extracted PID: " + pid);
				            } else {
				                System.out.println("PID not found in the URL");
				            }
	                    ...
	                    resultRow.createCell(2).setCellValue(name); 
	                    resultRow.createCell(3).setCellValue(size);
	                    resultRow.createCell(4).setCellValue(pid);
	                    resultRow.createCell(5).setCellValue(url);
	            ...
	            FileOutputStream outFile = new FileOutputStream(".\\Output\\Zepto Ny&Chri Output.xlsx");
